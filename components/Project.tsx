  ...
  return (
    <div className="min-w-0 flex-[1_1_330px]">
      {project.eyebrow ? (
        ...
        <MediaFrame asset={project.icon} variant="icon" />
      )}
      <h3
        id={headingId}
...
function PhoneWell({ project }: { project: ProjectData }) {
  return (
    <div className="flex min-w-0 flex-[1_1_400px] items-start justify-center gap-[clamp(14px,2vw,22px)] rounded-md bg-well p-[clamp(24px,3vw,40px)]">
      {project.screenshots.map((shot) => (
        <MediaFrame key={shot.label} asset={shot} variant="phone" />
      ))}
    </div>
  );
}

function CompactMedia({ project }: { project: ProjectData }) {
  return (
    <div className="flex min-w-0 flex-[1_1_300px] items-center gap-3.5">
      <MediaFrame asset={project.icon} variant="icon" />
      {project.screenshots.map((shot) => (
        <MediaFrame key={shot.label} asset={shot} variant="wide" />
      ))}
    </div>
  );
}
...
export function Project({ project }: { project: ProjectData }) {
  ...
  const media =
    project.layout === "compact" ? (
      <CompactMedia project={project} />
    ) : (
      <PhoneWell project={project} />
    );

  return (
    <article aria-labelledby={headingId} className={articleClass[project.layout]}>
